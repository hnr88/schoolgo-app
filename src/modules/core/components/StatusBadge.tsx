...
export function StatusBadge({ status, label, styles }: StatusBadgeProps) {
  ...
  return (
    <span className={cn('inline-flex h-6 items-center gap-1.5 rounded-pill px-2.5 text-xs font-semibold', resolved.bg, resolved.text)}>
      <span className={cn('inline-block h-1.5 w-1.5 rounded-pill', resolved.dot)} />
      {label}
    </span>
  );
}
