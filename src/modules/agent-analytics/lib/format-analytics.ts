const MAX_CHART_LABEL_LENGTH = 16;

export function formatOfferRate(rate: number, locale: string): string {
  return new Intl.NumberFormat(locale, {
    style: 'percent',
    maximumFractionDigits: 0,
  }).format(rate);
}

export function formatAvgDays(days: number | null, locale: string): string {
  if (days === null) return '—';
  return new Intl.NumberFormat(locale, { maximumFractionDigits: 1 }).format(days);
}

export function truncateChartLabel(label: string): string {
  if (label.length <= MAX_CHART_LABEL_LENGTH) return label;
  return `${label.slice(0, MAX_CHART_LABEL_LENGTH - 1)}…`;
}
