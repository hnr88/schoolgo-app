...
export function DashboardHeader() {
  ...
  return (
    <header className={`flex h-14 shrink-0 items-center gap-4 bg-card ${isSearchPage ? 'px-4' : 'px-6'}`}>
      <h1 className={`shrink-0 text-lg font-bold text-ink-900 ${isSearchPage ? 'w-80' : ''}`}>{t(`nav.${titleKey}`)}</h1>

      {isSearchPage && (
        <div className='flex-1'>
          <SearchBar className='px-4 py-1.5 shadow-none border-border' />
        </div>
      )}
