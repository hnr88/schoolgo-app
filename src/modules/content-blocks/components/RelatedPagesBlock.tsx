...
import { getContentStaticPageHref } from '@/modules/content-pages';
    ...
    <BlockShell id='related' eyebrow='Related' title='Continue with nearby pages' description='Related page cards give every design a natural next click.' tone='muted'>
      <div className='grid gap-5 md:grid-cols-3'>
        {relatedPages.map((page) => (
          <Link key={page.slug} href={getPageHref(page)} className='group overflow-hidden rounded-lg border border-border bg-card no-underline shadow-1 transition-transform duration-300 hover:-translate-y-1 hover:shadow-3'>
