...
const secondaryMetrics = [
  ['Response health', '92%', 'within target'],
  ['Evidence coverage', '8/10', 'ready fields'],
  ['Decision confidence', 'High', 'based on linked signals'],
] as const;
  ...
  return (
    ...
    >
      ...
      <div className='mt-5 grid gap-4 lg:grid-cols-3'>
        {secondaryMetrics.map(([label, value, note]) => (
          <div key={label} className='rounded-lg border border-background p-5'>
            ...
            <p className='mt-3 text-2xl font-bold text-background'>{value}</p>
            <p className='mt-1 text-sm text-background'>{note}</p>
          </div>
        ))}
      </div>
    </BlockShell>
