    ...
    <BlockShell eyebrow='Features' title='Reusable feature tiles' description='This block works for services, support areas, tools, benefits, and program highlights.'>
      <div className='grid gap-5 md:grid-cols-3'>
        {page.features.map((feature, index) => (
          <Link key={feature.title} href={feature.href ?? '#'} className='group overflow-hidden rounded-lg border border-border bg-card no-underline shadow-1 transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:shadow-3'>
            {index !== 1 ? (
            ...
            <span className='block p-6'>
              ...
              <span className='mt-4 block text-xs font-semibold uppercase text-primary'>{page.searchSignals[index]?.value ?? page.eyebrow}</span>
            </span>
          </Link>
        ))}
      </div>
