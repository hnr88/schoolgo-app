...
import { autocompleteAgentsQuerySchema } from '@/modules/agent-search/schemas/autocomplete-agents.schema';

export const dynamic = 'force-dynamic';

const STRAPI_AUTOCOMPLETE_URL = `${env.NEXT_PUBLIC_API_URL}/api/autocomplete/agents`;

export async function GET(request: NextRequest) {
  const limit = rateLimit(request, 'autocomplete-agents', {
    capacity: 60,
    refillPerSecond: 4,
  });
  if (!limit.ok) {
  ...
  const parsed = autocompleteAgentsQuerySchema.safeParse({
  ...
  try {
    ...
    console.error('[GET /api/autocomplete/agents] Proxy to Strapi failed');
    return NextResponse.json(
